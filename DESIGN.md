...
video/（原始绿幕 mp4 + 水印 mask；源视频从 Releases assets-videos 下载）
  ...
  → encode_thumbs.py     转码 640×360 VP9-alpha WebM → step04/（thumb）
  → encode_hevc_alpha.sh 生成 WebKit 用 HEVC-alpha MOV → dsh-pet/assets/thumb/
```
...
- step04 WebM 同步到 `dsh-pet/assets/thumb/`，在 macOS 10.15+ 运行 `encode_hevc_alpha.sh` 生成同名 MOV（npm 包自包含两种播放资源）
...
│   ├── thumb/*.{webm,mov}  # 640×360 播放变体（Chromium/Firefox + Apple WebKit）
...
- 注册 `/pet/` 前缀路由（`ctx.webServer.register`）：
  - `/pet/thumb/<name>.<webm|mov>` → 读用户动画目录，未命中时回退 `assets/thumb/`
...
- **引擎格式分流**：Apple WebKit（Safari / WKWebView / iOS 浏览器）请求 HEVC-alpha MOV；Chromium / Firefox 请求 VP9-alpha WebM
...
2. step04 WebM → dsh-pet/assets/thumb/；macOS 运行 encode_hevc_alpha.sh 生成同名 MOV
3. prepack-check.js          npm publish 前健康检查
4. npm pack                  检查包含双格式动画的 tarball
