...
// 分数由当前速度与宠物大小决定：速度越快 / 宠物越大，分越高（线性映射，可单测）。
...
/**
 * 点击积分：score = round(speed/100 × size/462)，至少 1 分。
 * speed ≤ 0 / size ≤ 0 返回 0（调用方按 0 不触发）。达标判定用 SCORE_MIN_SPEED 由调用方做。
 */
export const clickScore = (speed: number, size: number): number => {
  if (speed <= 0 || size <= 0) return 0;
  return Math.max(1, Math.round((speed / SCORE_SPEED_PER_POINT) * (size / PET_REF_WIDTH)));
};
